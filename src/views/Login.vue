...
const handleLogin = async () => {
  ...
  try {
    ...
    errorMsg.value = Array.isArray(detail)
      ? detail[0]?.msg
      : err.response?.data?.message || "身份验证失败";
  } finally {
...
<template>
  <div
    class="min-h-screen flex items-center justify-center p-6 bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-slate-50 to-blue-50/50"
  >
    <div class="w-full max-w-md card-glass p-8 space-y-8">
      <div class="text-center">
        <h1 class="text-3xl font-bold text-primary tracking-tight">U-Drop</h1>
        <p
          class="text-xs font-mono text-secondary mt-2 uppercase tracking-widest"
        >
          Secure Access
        </p>
      </div>
      ...
      <div
        class="flex items-center space-x-3 px-1 cursor-pointer group select-none"
      ...
      >
        <div
          :class="[
            ...
            rememberMe
              ? 'shadow-[0_0_15px_rgba(0,163,255,0.4)] scale-110'
              : 'bg-white/50 border-secondary/20 group-hover:border-primary/50',
          ]"
          :style="
            rememberMe
              ? {
        ...
        >
          <Check
            v-if="rememberMe"
            class="w-3.5 h-3.5 text-white stroke-[4px]"
          />
        </div>
        <div class="flex flex-col">
          <span class="text-xs font-bold text-on-surface">记住我</span>
          <span class="text-[10px] text-secondary/80 uppercase tracking-tighter"
            >开启长效会话 (滑动窗口续期)</span
          >
        </div>
      </div>
      ...
      <div
        v-if="errorMsg"
        class="text-red-500 text-xs text-center font-medium animate-pulse"
      >
        {{ errorMsg }}
      </div>
