...
const isStepValid = computed(() => {
  if (currentStep.value === 1) {
    return (
      formData.value.account.length >= 4 && formData.value.account.length <= 32
    );
  }
...
<template>
  <div
    class="min-h-screen flex items-center justify-center p-4 md:p-8 bg-[#aed9f4]"
  >
    <!-- Background Decor -->
    <div class="fixed inset-0 overflow-hidden pointer-events-none">
      <div
        class="absolute -top-[10%] -left-[10%] w-[40%] h-[40%] rounded-full bg-white/20 blur-[120px]"
      ></div>
      <div
        class="absolute -bottom-[10%] -right-[10%] w-[40%] h-[40%] rounded-full bg-primary/20 blur-[120px]"
        :style="{ backgroundColor: 'var(--accent-color)22' }"
      ></div>
    </div>
    ...
    <div class="w-full max-w-md relative z-10">
      ...
      <div class="text-center mb-8 md:mb-12">
        <div
          class="w-16 h-16 md:w-20 md:h-20 bg-white rounded-[24px] md:rounded-[30px] mx-auto flex items-center justify-center shadow-key mb-4 md:mb-6 animate-float"
        >
          <ShieldCheck
        ...
        <h1
          class="text-2xl md:text-3xl font-black text-primary-dark tracking-tighter uppercase mb-2"
        >
          System Setup
        </h1>
        <p
          class="text-[10px] md:text-xs font-mono text-secondary uppercase tracking-[0.3em] opacity-60"
        >
        ...
        <div class="flex justify-center space-x-3 mb-10">
          <div
            ...
            :class="[
              ...
              currentStep === i
                ? 'w-8 bg-primary shadow-[0_0_10px_rgba(0,163,255,0.5)]'
                : 'w-2 bg-white/30',
            ]"
            :style="
              currentStep === i
          ...
          <Transition :name="slideDirection" mode="out-in">
            ...
            <div v-if="currentStep === 1" :key="1" class="space-y-6">
              <div class="space-y-2">
                ...
                <p class="text-xs text-secondary/85">
                  此账号将拥有系统的最高权限，请谨慎设置。
                </p>
              </div>
              <div class="group">
                <label
                  class="block text-[10px] font-black text-secondary/75 uppercase tracking-widest mb-2 ml-1"
                  >Admin Account</label
                >
                <div class="relative">
                  <User
                    class="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-secondary/75 group-focus-within:text-primary transition-colors"
                  />
                  <input
            ...
            <div v-else-if="currentStep === 2" :key="2" class="space-y-6">
              ...
              <div class="space-y-4">
                ...
                <div class="px-1 space-y-2">
                  <div
                    class="flex justify-between text-[9px] font-black uppercase tracking-tighter"
                  >
                  ...
                  <div
                    class="h-1 w-full bg-black/5 rounded-full overflow-hidden"
                  >
                    <div
            ...
            <div v-else-if="currentStep === 3" :key="3" class="space-y-6">
              <div class="space-y-2">
                ...
                <p class="text-xs text-secondary/85">
                  初始化完成后，您可以在管理面板中随时修改。
                </p>
              </div>

              <div
                class="p-4 rounded-xl bg-black/5 border border-white/10 space-y-5"
              >
                <!-- Registration Toggle -->
                <div class="flex items-center justify-between">
                  <div class="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p class="text-[9px] text-secondary/85">
                        允许任何人访问您的主页并注册账号
                      </p>
                    </div>
                  </div>
                  <button
                    @click="
                      formData.allow_registration = !formData.allow_registration
                    "
                    :class="[
                    ...
                    :style="{
                      backgroundColor: formData.allow_registration
                        ? 'var(--accent-color)'
                        : '',
                    }"
                  >
                    <div
                      :class="[
                        ...
                        formData.allow_registration ? 'left-5' : 'left-1',
                      ]"
                    ></div>
                  </button>
                </div>
                ...
                <div class="pt-4 border-t border-black/5 space-y-4">
                  <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center space-x-2">
                      ...
                      />
                      <span class="text-[8px] font-black text-secondary/80"
                        >次/分</span
                      >
                    </div>
                  </div>
                  <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center space-x-2">
                      <input
                        type="number"
                        :value="
                          Math.round(formData.default_token_expire / 3600)
                        "
                        @input="
                          formData.default_token_expire =
              ...
              <div
                class="flex items-start space-x-2 p-3 rounded-lg bg-orange-400/10 border border-orange-400/20"
              >
      ...
      <div class="mt-8 text-center space-y-1 opacity-30">
        ...
        >
          U-Drop v0.5.1 Deployment Wizard
        </p>
        <p class="text-[8px] font-mono text-secondary">
          © 2026 Ethereal Systems • Industrial Grade Security
        </p>
      </div>
    </div>
  </div>
