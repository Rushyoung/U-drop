...
const handleRestore = async (messageId) => {
  try {
    ...
    if (res.data.success) {
      ...
      trashMessages.value = trashMessages.value.filter(
        (m) => m.id !== messageId,
      );
      fetchUser();
    }
...
const handleEmptyTrash = async () => {
  const confirmed = await showConfirm({
    title: "彻底清空回收站",
    message:
      "确定要物理抹除回收站中的所有内容吗？此操作将立即释放存储配额且不可撤销。",
    isDanger: true,
    confirmText: "立即物理抹除",
  });
...
<template>
  <div class="flex-1 overflow-y-auto p-12 bg-transparent">
    <div class="max-w-5xl mx-auto space-y-8">
      ...
      <!-- Empty State -->
      <div
        v-else-if="trashMessages.length === 0"
        ...
        >
          <div class="flex items-center space-x-6 flex-1 min-w-0">
            <div class="p-4 bg-black/5 rounded-2xl flex-shrink-0">
              <FileIcon
                v-if="msg.type === 1"
                class="w-6 h-6 text-secondary/85"
              />
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-center space-x-3 mb-1">
                <span
                  class="text-[10px] font-black uppercase tracking-widest text-secondary/75"
                  >{{ msg.device_name }}</span
                >
                <span class="text-[10px] font-mono text-secondary/20"
                  >#{{ msg.id.toString(16).toUpperCase() }}</span
                >
              </div>
              <h4 class="font-bold text-primary-dark truncate text-lg">
                {{
                  ...
                  (msg.attachments.length > 0
                    ? msg.attachments[0].display_name
                    : "无内容")
                }}
              </h4>
              <div class="flex items-center space-x-4 mt-2">
                ...
                <div
                  ...
                  class="text-[10px] font-bold text-primary/40 uppercase tracking-tighter"
                >
          ...
          <div class="flex items-center space-x-2">
            ...
            >
              <Loader2
                v-if="isProcessing === msg.id"
                class="w-5 h-5 animate-spin"
              />
              <RefreshCcw
                v-else
                class="w-5 h-5 transition-transform group-hover/btn:-rotate-45"
              />
            </button>
          </div>
        </div>
