...
const handleDeviceNameUpdate = async () => {
  ...
  try {
    ...
    if (res.data.success) {
      ...
      const current = devices.value.find(
    ...
    <div class="max-w-5xl mx-auto space-y-12">
      ...
      <header class="flex justify-between items-end">
        <div class="space-y-2">
          ...
          <p class="text-sm text-secondary/85 font-medium italic">
            实时监控与管理所有已授权的访问终端
          </p>
        </div>
        <button
      ...
      <section class="space-y-6">
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div
            ...
            :class="{
              'ring-2 ring-primary/20 bg-white/60':
          ...
          >
            <div class="space-y-6">
              <div class="flex items-start justify-between">
                <div class="flex items-center space-x-4 min-w-0">
                  <div
                    class="p-4 bg-primary/10 rounded-2xl shrink-0"
                  ...
                  <div class="min-w-0 space-y-1">
                    <div class="flex items-center space-x-2">
                      <h4
                        class="font-black text-primary-dark truncate text-lg uppercase tracking-tight"
                      >
                        {{
                          device.device_name?.replace(/\s\[.*\]$/, "") ||
                          "未知终端"
                        }}
                      </h4>
                      <span
                        v-if="device.device_id === currentDeviceId"
                        class="text-[8px] font-black uppercase px-2 py-0.5 bg-primary text-white rounded-lg tracking-widest shadow-lg"
                    ...
                    <p
                      class="text-[10px] font-mono font-bold text-secondary/60 uppercase tracking-tighter"
                    >
              ...
              <div
                v-if="device.device_id === currentDeviceId"
                class="p-4 rounded-2xl bg-black/5 border border-white/10 space-y-3"
              >
                <div class="flex items-center justify-between">
                  <span
                    class="text-[9px] font-black text-secondary/60 uppercase tracking-widest flex items-center"
                  >
                    <Edit3 class="w-3 h-3 mr-1.5" /> 修改设备名称
                  </span>
                </div>
                <div
                  class="flex flex-col xs:flex-row items-stretch xs:items-center gap-3"
                >
                  <input
            ...
            <div
              class="flex flex-col sm:flex-row items-start sm:items-center justify-between pt-6 border-t border-black/5 gap-4"
            >
              <div class="flex items-center space-x-4">
                <div class="flex flex-col">
                  ...
                  <div class="flex items-center space-x-2">
                    <div
                      v-if="
                        Math.floor(Date.now() / 1000) - device.last_seen < 90
                      "
                      class="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse shadow-[0_0_8px_rgba(34,197,94,0.6)]"
                    ></div>
                    <span
                      :class="[
                        'text-xs font-bold',
              ...
              >
                <Loader2
                  v-if="isRevoking === device.device_id"
                ...
                <template v-else>
                  ...
                  <span
                    ...
                    >{{
                      device.device_id === currentDeviceId ? "登出" : "下线"
                    }}</span
                  >
                </template>
              </button>
      ...
      <!-- SECURITY FOOTER -->
      <section
        class="card-glass p-8 md:p-12 border-dashed border-primary/20 bg-primary/[0.02]"
      >
        <div class="max-w-2xl space-y-4">
          <div
            class="flex items-center space-x-3 text-primary"
            :style="{ color: 'var(--accent-color)' }"
          >
          ...
          <p class="text-sm text-secondary/85 leading-relaxed font-medium">
            U-Drop
            采用设备绑定的会话机制。如果您在列表中发现异常终端，请立即执行“下线”操作。该终端的所有访问票据将被立即作废。
          </p>
        </div>
      </section>
    </div>
