...
const hasPreHashed = computed(
  () => pendingFiles.value.length > 0 && hashingCount.value === 0,
);
...
defineExpose({
  externalSetFiles,
  // 兼容旧的单文件调用
  externalSetFile: (file) => externalSetFiles([file]),
});
</script>

<template>
  <div
    class="relative w-full flex flex-col items-center z-50 pointer-events-none"
  >
    <!-- Pending Files List -->
    <Transition
    ...
    >
      <div
        ...
        class="w-full max-w-lg mb-3 pointer-events-auto space-y-2 px-2"
      >
        <div
          v-for="(file, idx) in pendingFiles"
          :key="file.name + file.size"
          class="card-glass p-2 md:p-3 flex items-center space-x-3 shadow-key bg-white/90 backdrop-blur-2xl"
        >
          ...
          <div class="flex-1 min-w-0">
            <p
              class="text-[10px] md:text-xs font-medium text-on-surface truncate"
            >
              {{ file.name }}
            </p>
            <p
              class="text-[8px] md:text-[9px] text-secondary/85 font-mono uppercase"
            >
          ...
          <button
            ...
            class="p-1 hover:bg-red-50 text-secondary/75 hover:text-red-500 rounded-full transition-colors shrink-0"
          >
        ...
        <div class="px-4 py-1 flex items-center justify-between">
          <div class="flex items-center space-x-2">
            <span
              v-if="isPreHashing"
              class="text-[8px] md:text-[10px] text-primary animate-pulse flex items-center"
            >
              <RotateCw class="w-2 h-2 mr-1 animate-spin" /> 正在预检
              {{ hashingCount }} 个文件...
            </span>
            <span
              v-if="hasPreHashed"
              class="text-[8px] md:text-[10px] text-green-600 flex items-center font-medium"
            >
              <CheckCircle2 class="w-2 h-2 mr-1" />
              {{ pendingFiles.length }} 就绪
            </span>
          </div>
          <button
            v-if="pendingFiles.length > 1"
            @click="clearAllFiles"
            class="text-[8px] md:text-[10px] text-secondary/75 hover:text-secondary underline decoration-dotted"
          >
            清除
          </button>
        </div>
    ...
    <!-- Main Input Bar -->
    <div
      class="w-full max-w-2xl flex items-center space-x-2 md:space-x-3 pointer-events-auto p-2 md:p-4"
    >
      <button
      ...
      <div class="flex-1 relative group">
        <input
          ...
          :placeholder="
            pendingFiles.length > 0 ? '描述...' : '输入指令或发送文件...'
          "
        />
      </div>
