...
<template>
  <div
    ref="containerRef"
    class="fixed inset-0 z-[500] flex items-center justify-center overflow-hidden touch-none"
  >
    <!-- Dark Backdrop -->
    <div
    ...
    <div
      v-if="!isImageLoaded"
      class="absolute inset-0 flex flex-col items-center justify-center space-y-6 z-[510] pointer-events-none"
    >
      <div class="relative">
        <div
          class="w-16 h-20 md:w-20 md:h-24 border-2 border-white/5 rounded-full animate-ping"
        ></div>
        <Loader2
          class="absolute inset-0 w-20 h-20 md:w-24 md:h-24 text-primary animate-spin"
          :style="{ color: 'var(--accent-color)' }"
        />
        <Maximize2
          class="absolute inset-0 m-auto w-6 h-6 md:w-8 md:h-8 text-white/40 animate-pulse"
        />
      </div>
      <div class="flex flex-col items-center space-y-1 px-4 text-center">
        <span
          class="text-[10px] md:text-xs font-black text-white uppercase tracking-[0.4em] md:tracking-[0.6em] animate-pulse"
          >Restoring High-Res</span
        >
        <span
          class="text-[8px] md:text-[9px] font-mono text-white/30 uppercase tracking-widest"
          >Downloading raw source data...</span
        >
      </div>
    </div>
    ...
    <div
      ...
      :style="{
        ...
        transform: isSwiping ? `translateY(${-lengthY}px)` : '',
      }"
    >
      <img
        :src="imageSrc"
    ...
    >
      ...
      >
        ...
        <div
          class="md:hidden text-[8px] text-white/30 font-mono uppercase tracking-[0.3em] animate-pulse mb-2"
        >
          Swipe to close • Tap image to hide UI
        </div>

        <div
          class="flex items-center space-x-2 md:space-x-4 px-4 md:px-6 py-2.5 md:py-3 card-glass bg-white/5 border-white/10 shadow-2xl backdrop-blur-2xl shrink-0 mx-4"
        >
          <span
            class="hidden sm:inline text-[10px] text-white/50 font-mono mr-2 md:mr-4 truncate max-w-[120px] md:max-w-[240px] uppercase tracking-tighter"
            >{{ fileName }}</span
          >
