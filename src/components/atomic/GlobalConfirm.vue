...
<template>
  ...
  >
    <div
      ...
      class="fixed inset-0 z-[1000] flex items-center justify-center p-6 bg-black/20 backdrop-blur-sm"
      ...
      >
        ...
        >
          ...
          <button
            ...
            class="absolute top-6 right-6 p-2 text-secondary/50 hover:text-secondary transition-colors"
          >
            <X class="w-5 h-5" />
          </button>
          ...
          <div class="flex items-start space-x-4">
            <div
              :class="[
                'p-3 rounded-2xl shrink-0 shadow-sm border border-white/40',
                confirmState.isDanger
                  ? 'bg-red-500/5 text-red-500'
                  : 'bg-primary/5 text-primary',
              ]"
              :style="
                !confirmState.isDanger ? { color: 'var(--accent-color)' } : {}
              "
            >
              <AlertTriangle
                v-if="confirmState.isDanger"
                class="w-5 h-5 md:w-6 md:h-6"
              />
              <Info v-else class="w-5 h-5 md:w-6 md:h-6" />
            </div>
            <div class="space-y-2">
              <h3 class="text-xl font-black text-primary-dark tracking-tight">
                {{ confirmState.title }}
              </h3>
          ...
          <div class="flex space-x-3 pt-4">
            ...
            <button
              ...
              :class="[
                'flex-1 px-6 py-4 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl transition-all active:scale-[0.98]',
                confirmState.isDanger
          ...
          <AlertTriangle
            v-if="confirmState.isDanger"
            class="absolute -right-6 -bottom-6 w-32 h-32 text-red-500/5 rotate-12 pointer-events-none"
          />
        </div>
      </Transition>
    </div>
