...
const handleContextMenu = (e) => {
  const options = [
    ...
    },
    {
      ...
      action: () => {
        showToast(
          `PK: 0X${props.message.id.toString(16).toUpperCase()}`,
          "info",
        );
      },
    },
...
const isOwn = computed(() => {
  ...
  return (
    props.message.device_name.includes(`[${shortId}]`) || props.message.id < 0
  );
});
...
const isUploading = computed(
  () => !!uploadTask.value && !uploadTask.value.isComplete,
);
...
const loadingRawHashes = ref(new Set());
...
const parsedContent = computed(() => {
  ...
  const urlRegex =
    /((?:https?:\/\/|www\.)[^\s]+|[a-z0-9-]+(?:\.[a-z0-9-]+)*\.(?:com|net|org|cn|io|me|info|biz|dev|cc|tv|icu|top|xyz|link)(?:\/[^\s]*)?|(?:\d{1,3}\.){3}\d{1,3}(?::\d+)?(?:\/[^\s]*)?)/gi;
  ...
  <div v-bind="$attrs">
    <div
      :class="[
        ...
        isOwn
          ? 'justify-end pl-[5%] md:pl-[25%]'
          : 'justify-start pr-[5%] md:pr-[25%]',
      ]"
    ...
    >
      <div
        :class="[
          'flex flex-col relative max-w-[85vw] md:max-w-full',
          isOwn ? 'items-end' : 'items-start',
        ]"
      >
        <!-- Sender Info -->
        <div
          class="flex items-center space-x-2 mb-1.5 px-1 text-[10px] md:text-[11px] font-black text-neutral-rich transition-all duration-500 uppercase opacity-70 md:opacity-100"
        >
        ...
        >
          ...
          <div class="relative z-10 text-white">
            ...
            <div
              ...
              class="text-[14px] md:text-[16px] leading-relaxed font-bold drop-shadow-md mb-3 md:mb-4 last:mb-0 break-words whitespace-pre-wrap overflow-hidden"
            >
            ...
            <div v-if="message.type === 1" class="space-y-3 md:space-y-4">
              ...
              >
                <!-- Image Attachment -->
                <div
                  v-if="isSupportedThumbnail(att.file_info.mime_type)"
                  @click="
                    handleImageClick(att.id, att.file_hash, att.display_name)
                  "
                  class="cursor-pointer"
                >
                  <div
                    class="relative rounded-xl md:rounded-2xl overflow-hidden bg-black/10 min-h-[100px] md:min-h-[120px] w-full md:min-w-[200px] flex items-center justify-center border border-white/20 shadow-inner group/img"
                  >
                    <!-- Local Loading Overlay -->
                    <div
                      v-if="loadingRawHashes.has(att.file_hash)"
                      class="absolute inset-0 z-20 bg-primary/20 backdrop-blur-sm flex flex-col items-center justify-center space-y-2"
                    >
                      <Loader2
                        class="w-5 h-5 md:w-6 md:h-6 text-white animate-spin"
                      />
                      <span
                        class="text-[7px] md:text-[8px] font-black uppercase tracking-widest animate-pulse"
                        >Fetching Raw</span
                      >
                    </div>

                    <img
                      v-if="thumbnailUrls[att.file_hash]"
                      :src="thumbnailUrls[att.file_hash]"
                      class="max-w-full max-h-[280px] md:max-h-[360px] object-cover transition-all duration-1000 ease-spring group-hover/img:scale-[1.05]"
                    />
                    <div
                      v-else
                      class="flex flex-col items-center space-y-2 opacity-20 p-6 md:p-8 text-white font-mono text-[9px] md:text-[10px]"
                    >
                      PREVIEW
                    </div>
                  </div>
                  <div class="flex justify-between items-center mt-1 px-1">
                    <p
                      class="text-[9px] md:text-[10px] text-white/60 truncate italic"
                    >
                      {{ att.display_name }}
                    </p>
                    <button
                      ...
                      "
                      class="opacity-0 md:group-hover/att:opacity-100 p-1 hover:bg-white/10 rounded transition-all"
                    >
                ...
                <div
                  v-else
                  @click="handleDownload(att.id, att.display_name)"
                  class="flex items-center space-x-3 md:space-x-4 p-2.5 md:p-3 bg-black/5 rounded-lg md:rounded-xl border border-white/10 cursor-pointer hover:bg-black/10 transition-all"
                >
                  ...
                  <div class="flex-1 min-w-0">
                    <h4
                      class="font-bold text-[12px] md:text-[13px] truncate text-white"
                    >
                      {{ att.display_name }}
                    </h4>
                    <p
                      class="text-[8px] md:text-[9px] text-white/60 font-mono mt-0.5"
                    >
                      {{
                        (att.file_info.file_size / 1024 / 1024).toFixed(2)
                      }}
                      MB
                    </p>
                  </div>
              ...
              <div v-if="isUploading" class="pt-1.5 md:pt-2">
                <div
                  class="h-1 w-full bg-black/10 rounded-full overflow-hidden"
                >
                  <div
                    class="h-full bg-white shadow-[0_0_10px_#fff] transition-all duration-500"
                    :style="{ width: `${uploadProgress}%` }"
                  ></div>
                </div>
                <div class="flex justify-between items-center mt-1.5">
                  <span
                    class="text-[7px] md:text-[8px] font-black italic uppercase text-white/50 tracking-tighter"
                    >Transmission</span
                  >
                  <p
                    class="text-[8px] md:text-[10px] text-white font-black italic uppercase tracking-widest drop-shadow-md"
                  >
                    {{ uploadProgress }}%
                  </p>
                </div>
            ...
            <!-- Metadata Footer -->
            <div
              class="flex items-center justify-between mt-3 md:mt-5 border-t border-white/10 pt-3 md:pt-4"
            >
              <div
                class="flex items-center space-x-1.5 md:space-x-2 text-white/80"
              >
                <Clock class="w-3 h-3 md:w-3.5 md:h-3.5 shadow-sm" />
                <span
                  class="text-[10px] md:text-[12px] font-mono font-black tracking-tighter md:tracking-normal"
                  >{{ formattedTime }}</span
                >
              </div>
              <span
                class="text-[8px] md:text-[9px] font-mono text-white/60 tracking-tighter uppercase"
              >
                #{{
                  message.id < 0
                    ? "STAGED"
                    : message.id.toString(16).toUpperCase()
                }}
              </span>
            </div>
          </div>
    ...
    <Teleport to="body">
      <ImageOverlay
        v-if="showFullImage && activeImageId"
        :attachment-id="activeImageId"
        :file-name="activeImageName || 'image.png'"
        :file-hash="activeImageHash || ''"
