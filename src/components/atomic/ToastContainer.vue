...
<template>
  <div class="fixed top-8 left-1/2 -translate-x-1/2 z-[100] flex flex-col space-y-3 pointer-events-none items-center w-full max-w-xl">
    <TransitionGroup 
    ...
    >
      ...
      >
        ...
        <div :class="[
          ...
          toast.type === 'error' ? 'bg-red-500' : (toast.type === 'success' ? 'bg-green-500' : 'bg-primary')
        ]"></div>

        <div class="flex-shrink-0 mt-0.5">
          <AlertCircle v-if="toast.type === 'error'" class="w-5 h-5 text-red-500" />
          <CheckCircle2 v-else-if="toast.type === 'success'" class="w-5 h-5 text-green-500" />
          <Info v-else class="w-5 h-5 text-primary" />
        </div>

        <div class="flex-1">
          <p class="text-sm font-medium text-on-surface leading-tight">{{ toast.message }}</p>
        </div>

        <button @click="remove(toast.id)" class="text-secondary/40 hover:text-on-surface transition-colors">
          <X class="w-4 h-4" />
        </button>
      </div>
    </TransitionGroup>
