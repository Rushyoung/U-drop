...
const shouldShowDateSeparator = (idx) => {
  ...
  const curr = new Date(
    filteredMessages.value[idx].timestamp * 1000,
  ).toDateString();
  const prev = new Date(
    filteredMessages.value[idx - 1].timestamp * 1000,
  ).toDateString();
  return curr !== prev;
};
...
const filteredMessages = computed(() => {
  ...
  return messages.value.filter((m) => {
    ...
    const attachmentMatch = m.attachments.some((a) =>
...
const updateReadingProgress = useDebounceFn(() => {
  if (!containerRef.value || isInitialPositioning.value || isLoading.value)
    return;
...
watch(containerY, () => {
  if (containerRef.value) {
    const threshold =
      containerRef.value.scrollHeight - containerRef.value.clientHeight - 400;
...
watch(
  ...
  (newLen, oldLen) => {
    if (
      newLen > oldLen &&
      !isInitialPositioning.value &&
      _settings.autoScrollToBottom
    ) {
      const container = containerRef.value;
      if (container) {
...
const scrollToAnchor = (anchorId, behavior = "auto") => {
 ...
 * 带锚定保护的加载逻辑
 */
const loadMessagesWithAnchoring = async (mode, anchor) => {
...
const handleLoadMoreAfter = async () => {
  if (isLoading.value) return;
  await loadMessagesWithAnchoring(
    "after",
...
<template>
  <div
    class="flex-1 relative m-2 md:m-4 overflow-hidden flex flex-col"
  ...
  >
    ...
    >
      <div
        v-if="isInitialPositioning"
        class="absolute inset-0 z-50 bg-[#aed9f4] flex items-center justify-center rounded-[16px]"
        :style="{ backgroundColor: 'var(--surround-color)' }"
      >
        <div class="flex flex-col items-center space-y-6">
          <div class="relative">
            <div
              class="w-16 h-16 border-4 border-primary/10 rounded-full"
              :style="{
                borderColor:
            ...
            ></div>
            <Loader2
              class="absolute inset-0 w-16 h-16 text-primary animate-spin"
              :style="{ color: 'var(--accent-color)' }"
            />
          </div>
          <div class="flex flex-col items-center space-y-1">
            <span
              class="text-xs font-black text-primary uppercase tracking-[0.5em] animate-pulse"
              :style="{ color: 'var(--accent-color)' }"
              >Restoring State</span
            >
            <span
              class="text-[9px] font-mono text-primary/40 uppercase"
              :style="{
                color:
                  'color-mix(in srgb, var(--accent-color) 40%, transparent)',
              }"
              >Synchronizing Ledger Data</span
            >
          </div>
        </div>
      </div>
    ...
    >
      ...
      >
        ...
        <div
          ref="topAnchor"
          class="h-10 w-full flex flex-col justify-center items-center relative overflow-hidden mb-4"
        >
          <div
            v-if="isLoading"
            class="absolute inset-x-0 top-0 h-1 bg-primary/10 overflow-hidden"
          >
            <div
              class="h-full bg-primary animate-scan w-1/2 shadow-[0_0_15px_#00A3FF]"
            ></div>
          </div>
          <div
            v-if="isLoading"
            class="text-[9px] font-black text-primary uppercase tracking-[0.4em] animate-pulse"
          >
            Retrieving History
          </div>
        </div>
        ...
        <div
          v-if="searchQuery"
          class="mb-6 px-4 py-2 bg-white/5 rounded-xl border border-white/10 flex items-center justify-between"
        >
          <div
            class="flex items-center space-x-2 text-[10px] font-bold text-white/80 uppercase tracking-widest"
          >
            <Search class="w-3 h-3" />
            <span>Filtering Cache</span>
          </div>
          <span
            class="text-[10px] font-mono text-primary animate-pulse"
            :style="{ color: 'var(--accent-color)' }"
          >
        ...
        <div class="space-y-4">
          ...
          <div
            v-if="
              ...
              !isInitialPositioning
            "
            class="flex flex-col items-center justify-center py-20 animate-in fade-in zoom-in duration-1000"
          >
            <div class="relative mb-6">
              <div
                class="w-24 h-24 bg-white/10 rounded-[40px] flex items-center justify-center shadow-key border border-white/10"
              >
                <History class="w-10 h-10 text-white/60" />
              </div>
              <div
                class="absolute -bottom-2 -right-2 w-8 h-8 bg-primary/40 rounded-full blur-xl animate-pulse"
                :style="{ backgroundColor: 'var(--accent-color)' }"
              ></div>
            </div>
            <div class="text-center space-y-2">
              <h3
                class="text-lg font-black text-white/70 uppercase tracking-[0.3em]"
              >
                你已抵达世界尽头
              </h3>
              <p
          ...
          <template v-for="(msg, idx) in filteredMessages" :key="msg.id">
            ...
            <div
              ...
              class="flex items-center justify-center my-8 md:my-12"
            >
              <div class="h-px flex-1 bg-white/5 shadow-inner"></div>
              <div
                class="px-4 py-1.5 rounded-full bg-white/5 border border-white/5 backdrop-blur-md shadow-ambient mx-4"
              >
                <span
                  class="text-[10px] md:text-[11px] font-black uppercase tracking-[0.2em] text-white/80 drop-shadow-sm"
                >
            ...
            <div
              :class="[
                'transition-all duration-500 flex items-center group/row',
                isBulkMode ? 'pl-4' : '',
              ]"
            >
              <!-- 批量选择图标按钮 -->
        ...
        >
          <div
            v-if="isLoading"
            class="w-4 h-4 border-2 border-primary border-t-transparent rounded-full animate-spin opacity-20"
          ></div>
        </div>
      </div>
    </div>
    ...
    >
      <div
        ...
        class="absolute bottom-8 left-1/2 -translate-x-1/2 z-[100] w-auto"
      >
        <div
          class="card-glass px-6 py-4 flex items-center space-x-8 shadow-2xl border-white/10 bg-black/40 backdrop-blur-2xl"
        >
          <div class="flex flex-col">
            <span
              class="text-[9px] font-black uppercase text-white/80 tracking-widest leading-none mb-1"
              >Batch Manager</span
            >
