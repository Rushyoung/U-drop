...
function getHEAPU8(module) {
  ...
  if (!buffer && module.asm && module.asm.memory)
    ...
    try {
      ...
      if (size <= blockSize * 3) {
        ...
        buffers.push(
          await chunk
            .slice(
              Math.floor((size - blockSize) / 2),
              Math.floor((size - blockSize) / 2) + blockSize,
            )
            .arrayBuffer(),
        );
      ...
      const calculateMD5 = module.cwrap("CalculateFastFileMD5", "number", [
        "string",
        "number",
      ]);
    ...
    try {
      ...
      while (offset < totalSize) {
        ...
        if (counter % 8 === 0) {
          ...
          console.log(
            `[Hash Worker] BLAKE3 Progress: ${progress}% (${(offset / 1024 / 1024).toFixed(0)}MB)`,
          );
        }
      }
      ...
      console.log(
        `[Hash Worker] BLAKE3 Finished. Speed: ${(totalSize / 1024 / 1024 / (internalTime / 1000)).toFixed(2)} MB/s`,
      );
      self.postMessage({ id, hash, internalTime });
    } catch (error) {
