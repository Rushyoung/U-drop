  ...
  const uploadSingleFile = async (
    ...
    fullHashPromise
      .then((res) => {
        ...
        console.log(
          `[Upload Diagnostic] Parallel Hash READY for ${file.name}: ${res.hash}`,
        );
      })
      .catch((err) => {
    ...
    try {
      ...
      console.log(
        `[Upload Diagnostic] Starting Immediate Upload for ${file.name} (${totalChunks} chunks)`,
      );
      ...
      for (let i = 0; i < totalChunks; i++) {
        ...
        try {
          const uploadRes = await FileService.uploadChunk(
            uploadId,
            chunk,
            currentHash,
          );
          if (uploadRes.data.data?.status === "deduplicated") {
            console.log(
              `[Upload Diagnostic] HIJACKED! Deduped at chunk ${i + 1}`,
            );
            isHijacked = true;
            break;
          }
      ...
      if (!isHijacked) {
        ...
        if (!calculatedFullHash) {
          console.log(
            `[Upload Diagnostic] Waiting for background hash to finish for COMMIT...`,
          );
        }
        const finalHashRes = await fullHashPromise;
        console.log(
          `[Upload Diagnostic] Executing COMMIT with hash: ${finalHashRes.hash}`,
        );
  ...
  const startUpload = async (files, content, onCreated) => {
    ...
    try {
      ...
      if (!res.data.success || !res.data.data)
        throw new Error("Backend rejected task");
