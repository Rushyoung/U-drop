...
import { X, Clock, MousePointer2 as ClickIcon, Share2, Loader2, FileText } from 'lucide-vue-next';
...
const handleConfirm = () => {
  const finalFileName = hasExtension.value && editableExtension.value 
...
<template>
  <div class="fixed inset-0 z-[400] flex items-center justify-center p-6 bg-black/20 backdrop-blur-sm" @click.self="emit('close')">
    <div 
      class="w-full max-w-sm card-glass p-8 space-y-8 animate-focus relative overflow-hidden"
      style="background: linear-gradient(145deg, #f0f4f8, #d9e2ec); border-radius: 32px; border: 1px solid rgba(255,255,255,0.8);"
    >
      <!-- Close Button -->
      <button @click="emit('close')" class="absolute top-6 right-6 p-2 text-secondary/75 hover:text-secondary transition-colors">
        <X class="w-5 h-5" />
      </button>

      <div class="space-y-6">
        <h3 class="text-xl font-black text-primary-dark flex items-center space-x-3">
          <Share2 class="w-6 h-6" />
          <span>配置分享权限</span>
        </h3>
        
        <div class="group">
          <label class="block text-[10px] font-black text-secondary/80 uppercase tracking-widest mb-2 ml-1">显示名称 (Display Name)</label>
          
          <!-- Single Input if no extension -->
          <div v-if="!hasExtension" class="relative">
            <FileText class="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-secondary/40 group-focus-within:text-primary transition-colors" />
            <input 
          ...
          <div v-else class="flex items-center space-x-2">
            <div class="flex-1 relative group/name">
              <FileText class="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-secondary/40 group-focus-within/name:text-primary transition-colors" />
              <input 
                v-model="editableBaseName"
                type="text"
                placeholder="主文件名"
      ...
      <div class="space-y-6">
        <!-- Expire In -->
        <div class="space-y-3">
          <div class="flex items-center space-x-2 text-[10px] font-black uppercase tracking-widest text-secondary/80">
            <Clock class="w-3 h-3" />
            <span>有效期 (Expiration)</span>
          </div>
          <div class="grid grid-cols-2 gap-2">
            <button 
              v-for="opt in expireOptions" :key="opt.label"
              @click="selectedExpire = opt.value as any"
              :class="[
                'px-4 py-2.5 rounded-xl text-xs font-bold transition-all active:scale-95',
                selectedExpire === opt.value ? 'bg-primary text-white shadow-lg' : 'bg-white/40 text-secondary hover:bg-white/60'
              ]"
              :style="selectedExpire === opt.value ? { backgroundColor: 'var(--accent-color)' } : {}"
            >
              {{ opt.label }}
            </button>
          </div>
        ...
        <div class="space-y-3">
          ...
          <div class="grid grid-cols-2 gap-2">
            <button 
              v-for="opt in useOptions" :key="opt.label"
              @click="selectedUses = opt.value"
              :class="[
                'px-4 py-2.5 rounded-xl text-xs font-bold transition-all active:scale-95',
                selectedUses === opt.value ? 'bg-primary text-white shadow-lg' : 'bg-white/40 text-secondary hover:bg-white/60'
              ]"
              :style="selectedUses === opt.value ? { backgroundColor: 'var(--accent-color)' } : {}"
            >
              {{ opt.label }}
            </button>
          </div>
      ...
      <button 
        ...
        :style="{ backgroundColor: 'var(--accent-color)', boxShadow: '0 10px 30px rgba(0,163,255,0.3)' }"
      >
