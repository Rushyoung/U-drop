...
function getWorker(): Worker {
  if (!persistentWorker) {
    persistentWorker = new Worker(new URL('../workers/hash.worker.ts', import.meta.url), { type: 'module' });
 ...
 * 带有 ID 追踪和超时保护的哈希请求
 */
async function requestHash(type: string, chunk: Blob, timeoutMs: number = 30000): Promise<HashResult> {
...
export async function calculateSparseMd5(file: File): Promise<HashResult> {
  return requestHash('sparse-md5', file, 30000);
}

export async function calculateFullBlake3(file: File): Promise<HashResult> {
