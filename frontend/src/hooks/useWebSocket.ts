...
import type { UdropWSMessage } from '../types/api';
...
const currentSyncSeq = ref<number>(0);
const eventHandlers = new Set<(msg: UdropWSMessage) => void>();
...
export function useWebSocket() {
  const connect = () => {
    ...
    socket.value.onmessage = (event) => {
      try {
        const msg: UdropWSMessage = JSON.parse(event.data);
        handleMessage(msg);
      } catch (err) {
  ...
  const handleMessage = (msg: UdropWSMessage) => {
    if (msg.type === 'INIT') {
  ...
  const onMessage = (handler: (msg: UdropWSMessage) => void) => {
