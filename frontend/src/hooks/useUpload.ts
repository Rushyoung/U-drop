...
import { calculateSparseMd5, calculateFullBlake3, type HashResult, getMimeType } from '../utils/hash';
...
export function useUpload() {
  ...
  const uploadSingleFile = async (
    ...
    uploadId: string,
    file: File,
    fullHashPromise: Promise<HashResult>
  ) => {
    let calculatedFullHash: string | null = null;
   ...
   * 发起上传任务
   */
  const startUpload = async (files: File[], content?: string, onCreated?: (msgId: number) => void) => {
