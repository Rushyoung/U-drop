...
import type { MessageResponse, TimelineMode } from '../types/api';
 ...
 * 核心：幂等合并消息列表
 */
const upsertMessages = (newMsgs: MessageResponse[]) => {
  if (isDestroyed) return;

  console.log(`[Timeline Diagnostic] upsertMessages called with ${newMsgs.length} messages.`);
  const msgMap = new Map(messages.value.map(m => [m.id, m]));
  newMsgs.forEach(m => {
    console.log(`[Timeline Diagnostic] Merging ID: ${m.id}. Attachments: ${m.attachments?.length || 0}`);
    msgMap.set(m.id, m);
  });
  ...
  const loadMessages = async (mode: TimelineMode = 'initial', anchorId?: number | null) => {
    ...
    try {
      ...
      const targetAnchor = (mode === 'initial' && anchorId === undefined) 
        ? getLastAnchor() 
        : anchorId;
  ...
  const addOptimisticMessage = (msg: MessageResponse) => {
    upsertMessages([msg]);
  };
   ...
   */
  const updateMessageId = (oldId: number, newId: number) => {
    const index = messages.value.findIndex(m => m.id === oldId);
    if (index !== -1) {
      console.log(`[Timeline Diagnostic] PROMOTING ${oldId} -> ${newId}. Reference updated.`);
      // 关键：创建全新的对象引用，确保 v-for 的 :key 侦测到物理变更
      const updatedMsg = { ...messages.value[index], id: newId };
      messages.value[index] = updatedMsg;
      messages.value = [...messages.value]; 
    } else {
      console.warn(`[Timeline Diagnostic] Promotion FAILED: Temp ID ${oldId} not found.`);
    }
  };
  ...
  const cleanupWS = onMessage((msg) => {
    ...
    if (msg.sync_seq && msg.sync_seq > currentSyncSeq.value + 1 && currentSyncSeq.value !== 0) {
    ...
    switch (msg.type) {
      case 'MSG_NEW':
        if (msg.data?.message_id) {
          console.log(`[Timeline] Real-time NEW_MESSAGE: ${msg.data.message_id}`);
          refreshMessage(msg.data.message_id);
        }
        break;
      case 'MSG_DELETE':
        if (msg.data?.message_id) {
          console.log(`[Timeline] Real-time MESSAGE_DELETED: ${msg.data.message_id}`);
          removeMessage(msg.data.message_id);
        }
        break;
      case 'DEVICE_UPDATE':
        if (msg.data?.device_id && msg.data?.device_name) {
          console.log(`[Timeline] Real-time DEVICE_UPDATE: ${msg.data.device_id} -> ${msg.data.device_name}`);
  ...
  const loadMoreBefore = () => {
    if (isDestroyed || !hasMoreBefore.value || messages.value.length === 0) return;
  ...
  const loadMoreAfter = async () => {
    ...
    const newestId = realMsgs.length > 0 ? realMsgs[realMsgs.length - 1].id : null;
  ...
  const refreshMessage = async (messageId: number) => {
    try {
      console.log(`[Timeline Diagnostic] refreshMessage requesting ID: ${messageId}`);
      ...
      if (!isDestroyed && response.data.success && response.data.data) {
        console.log(`[Timeline Diagnostic] refreshMessage received ${response.data.data.length} items`);
