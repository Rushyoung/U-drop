...
class UserSettingsUpdate(BaseModel):
    """用户设置更新请求"""
    trash_expire_days: Optional[int] = Field(None, ge=1, le=365, description="回收站保留天数 (1-365)")
    temp_expire_hours: Optional[int] = Field(None, ge=1, le=24, description="临时 Session 寿命 (1-24小时)")
    sliding_window_days: Optional[int] = Field(None, ge=1, le=365, description="滑动窗口续期天数 (1-365)")

class DeviceUpdateRequest(BaseModel):
    """设备信息更新请求"""
    device_name: str = Field(..., min_length=1, max_length=50, description="新设备显示名称")
