...
async def create_message(
    ...
    logger.info(
        f"消息已创建: ID={message_id} | 来源={user_uuid[:8]} | 附件数={len(msg.files)}"
    )

    await ws_manager.broadcast_to_user(
        user_uuid, {"type": "MSG_NEW", "data": {"message_id": message_id}}
    )
    ...
    if msg.files:
        for file_intent in msg.files:
            ...
            upload_infos.append(
                UploadTaskInfo(upload_id=upload_id, file_name=file_intent.file_name)
            )
            logger.debug(
                f"  └─ 待上传: {file_intent.file_name} ({file_intent.mime_type or 'unknown'}) | Size: {file_intent.total_size} | TaskID: {upload_id[:8]}..."
            )

    return ResponseSchema.ok(
        data=MessageCreateResponse(
            status="accepted" if upload_infos else "created",
            message_id=message_id,
...
async def list_messages(
    mode: str = Query(
        "initial",
        description="initial: 初始加载, before: 向旧消息翻页, after: 探测新消息",
    ),
    ...
    results = message_service.list_timeline(
        user_uuid=session["user_uuid"],
        limit=limit,
        anchor_id=anchor_id,
        mode=mode,
...
@router.get(
    "/trash", response_model=ResponseSchema[List[MessageResponse]], summary="查看回收站"
)
async def get_trash(
    session=Depends(get_current_session),
    message_service: MessageService = Depends(get_message_service),
...
@router.post(
    "/{message_id}/restore", response_model=ResponseSchema[None], summary="恢复消息"
)
...
@router.delete(
    ...
    summary="清空回收站",
    description="彻底物理删除当前用户回收站内的所有消息及其附件，并全额释放存储配额。",
)
