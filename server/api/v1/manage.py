...
@router.get(
    "/settings", response_model=ResponseSchema[dict], summary="获取系统全局配置"
)
async def get_manage_settings(
...
@router.put(
    "/settings", response_model=ResponseSchema[None], summary="修改系统全局配置"
)
async def update_manage_settings(
...
@router.get(
    "/users",
    response_model=ResponseSchema[List[UserManageResponse]],
    summary="用户列表管理",
)
...
async def factory_reset(
    ...
    """验证管理员密码后，清空系统所有业务数据"""
    # 1. 验证密码
    if not AuthManager.verify_password_hash(
        req.admin_password, current_admin.password_hash
    ):
