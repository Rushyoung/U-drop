...
def register(req: LoginRequest, auth: AuthService = Depends(get_auth_service)):
    try:
...
async def update_current_device(
    ...
    await ws_manager.broadcast_to_user(
        ...
        {
            "type": "DEVICE_UPDATE",
            "data": {"device_id": session["device_id"], "device_name": req.device_name},
        },
        bump_seq=True,
    )
...
@router.get(
    "/devices",
    response_model=ResponseSchema[List[DeviceResponse]],
    summary="获取设备列表",
)
...
@router.delete(
    "/devices/{device_id}", response_model=ResponseSchema[None], summary="注销特定设备"
)
