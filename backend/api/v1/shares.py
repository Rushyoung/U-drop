...
from dependencies import get_share_service, get_current_session
from db.services.share_service import ShareService
from schemas.shares import ShareCreateRequest, ShareCreateResponse, ShareItemResponse
from schemas.base import ResponseSchema
from core.logger import logger

router = APIRouter(tags=["Phase 4: 分享中心"])

@router.post("/file", response_model=ResponseSchema[ShareCreateResponse], summary="为附件生成分享链接")
async def create_file_share_link(
    ...
    return ResponseSchema.ok(data=ShareCreateResponse(
        share_id=share_id,
        share_url=share_url,
        expire_time=expire_time
    ))

@router.get("/list", response_model=ResponseSchema[List[ShareItemResponse]], summary="列出我创建的分享")
async def list_my_shares(
    session = Depends(get_current_session),
    share_service: ShareService = Depends(get_share_service)
):
    rows = share_service.list_user_shares(session["user_uuid"])
    return ResponseSchema.ok(data=[ShareItemResponse.model_validate(dict(row)) for row in rows])

@router.delete("/{share_id}", response_model=ResponseSchema[None], summary="撤销分享链接")
