from fastapi import APIRouter, Depends, Request, Header, Query, Path as FastPath
...
from dependencies import get_file_service, get_message_service, get_current_session, get_auth_service
...
from core.exceptions import TaskNotFound, ForbiddenError, HashMismatch, InternalError
from core.logger import logger
from schemas.messages import AttachmentResponse, BigFileResponse
...
async def _bind_file_to_message(task: UploadTask, file_hash: str, message_service: MessageService, auth_service: AuthService):
    """内部辅助：绑定附件并同步配额，最后发送 WS 信号 (真实 CAS 计费)"""
    message_service.bind_file_to_message(task.user_uuid, task.message_id, file_hash, task.file_name)
    await ws_manager.broadcast_to_user(task.user_uuid, {"type": "MSG_NEW", "data": {"message_id": task.message_id, "file_hash": file_hash}})

@router.post("/upload/init", response_model=ResponseSchema[dict], summary="初始化极速分片上传")
async def init_upload(
    ...
    async with user_lock:
        ...
        upload_id = uploads_manager.create_task(
...
async def upload_file_chunked(
    request: Request, 
    upload_id: str,
    x_full_hash: Optional[str] = Header(None, pattern="^[a-fA-F0-9]{64}$", description="可选：文件的全量哈希，用于秒传劫持"),
    session = Depends(get_current_session),
    ...
    if not task or task.user_uuid != session["user_uuid"]: raise TaskNotFound()
    ...
    if x_full_hash:
        ...
        if existing_file:
            ...
            user_lock = await get_user_lock(session["user_uuid"])
            async with user_lock:
                await _bind_file_to_message(task, x_full_hash, message_service, auth_service)
                uploads_manager.remove_task(upload_id)
                if os.path.exists(task.temp_path):
                    os.remove(task.temp_path)
            logger.success(f"🚀 [秒传劫持] 命中: {x_full_hash[:16]}... | 消息: {task.message_id}")
            return ResponseSchema.ok(data={"status": "deduplicated", "file_hash": x_full_hash})
        ...
        async with aiofiles.open(task.temp_path, mode='ab') as f:
            ...
            async for chunk in request.stream():
                ...
                if curr + len(chunk) > task.total_size:
                    logger.error(f"上传溢出拦截: 任务 {upload_id} 超出声明大小 {task.total_size}")
            ...
            return ResponseSchema.ok(data={"received_size": curr, "total_size": task.total_size})
    except Exception as e:
        if isinstance(e, ForbiddenError): raise e
...
async def commit_upload(
    upload_id: str, 
    x_full_hash: str = Header(..., pattern="^[a-fA-F0-9]{64}$", description="文件的 Blake3 全量哈希"),
    session = Depends(get_current_session),
    file_service: FileService = Depends(get_file_service),
    message_service: MessageService = Depends(get_message_service),
    auth_service: AuthService = Depends(get_auth_service)
    ...
    """
    task = uploads_manager.get_task(upload_id)
    if not task or task.user_uuid != session["user_uuid"]: raise ForbiddenError()
    ...
    if server_hash != x_full_hash:
        logger.warning(f"哈希一致性失败! Client={x_full_hash[:10]}, Server={server_hash[:10]}")
        uploads_manager.remove_task(upload_id)
        raise HashMismatch()
    ...
    async with user_lock:
        if not file_service.get_file_by_hash(server_hash):
            logger.info(f"新文件物理落盘: {server_hash[:16]}... | 原名: {task.file_name}")
            await file_service.finalize_upload(task.temp_path, server_hash, task.sparse_hash, task.mime_type)
...
async def detach_attachment(
    ...
    user_lock = await get_user_lock(session["user_uuid"])
    async with user_lock:
        success = message_service.detach_attachment_safe(session["user_uuid"], message_id, attachment_id)
    
    if not success: raise TaskNotFound("附件或引用关系不存在")
...
async def download_attachment(
    ...
    """逻辑 ID 寻址下载：包含所有权校验"""
    # 1. 获取附件详情
    attach = message_service.attachments.get_attachment_by_id(attachment_id)
    if not attach: raise TaskNotFound("附件不存在")
    
    # 2. 所有权校验：检查附件所属消息的发送者
    msg = message_service.messages.get_by_id(attach['message_id'])
    if not msg or msg['sender_uuid'] != session['user_uuid']:
        logger.warning(f"越权下载拦截 | 用户 {session['user_uuid'][:8]} 试图下载附件 {attachment_id}")
    ...
    if not path or not path.exists(): raise TaskNotFound("物理文件已丢失")
    
    logger.info(f"用户 {session['user_uuid'][:8]} 下载附件: {attach['display_name']} (Hash: {full_hash[:8]}...)")
    return FileResponse(
        path=path, 
...
async def get_attachment_thumbnail(
    ...
    if not file_info: raise TaskNotFound()
    ...
    if not path or not os.path.exists(path): raise TaskNotFound()
