...
class UdropWSMessage(BaseModel):
    """WebSocket 统一信令格式"""
    type: str = Field(..., description="信令类型: INIT, MSG_NEW, MSG_DELETE, UPLOAD_PROGRESS, DEVICE_UPDATE, PONG")
    sync_seq: Optional[int] = Field(None, description="自增同步序列号")
    data: Optional[Union[WSInitData, WSMsgNewData, WSMsgDeleteData, WSUploadProgressData, WSDeviceUpdateData, Dict[str, Any]]] = Field(None, description="信令负载数据")
