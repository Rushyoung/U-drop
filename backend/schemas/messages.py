...
class FileInfoShort(BaseModel):
    ...
    full_hash: str = Field(..., pattern="^[a-fA-F0-9]{64}$", description="文件的 Blake3 全量哈希值", json_schema_extra={"example": "4b1f8719..."})
    file_size: int = Field(..., description="文件大小 (Bytes)", json_schema_extra={"example": 266684})
    mime_type: Optional[str] = Field(None, description="文件 MIME 类型", json_schema_extra={"example": "image/png"})
...
class MessageCreateRequest(BaseModel):
    """创建消息请求 (支持多附件)"""
    content: Optional[str] = Field(None, description="文本内容", json_schema_extra={"example": "Hello!"})
    type: Optional[Literal[0, 1]] = Field(None, description="指定展示类型 (0:纯文本, 1:混合/附件)")
    files: List[FileUploadIntent] = Field(default_factory=list, description="要上传的文件列表")
...
class BigFileResponse(BaseModel):
    ...
    references: List[BigFileReference] = Field(default_factory=list, description="当前用户对该文件的详细引用清单")

class MessageResponse(BaseModel):
    """完整消息响应模型 (解耦+幻影聚合)"""
    model_config = ConfigDict(from_attributes=True)
    id: int = Field(..., description="消息唯一 ID")
    type: int = Field(..., description="展示类型 (0:纯文本, 1:混合)", json_schema_extra={"example": 1})
    ...
    # 附件展示
    attachments: List[AttachmentResponse] = Field(default_factory=list, description="已完成入库的附件")
    pending_uploads: List[PendingUploadResponse] = Field(default_factory=list, description="正在上传中的附件幻影")
...
class MessageCreateResponse(BaseModel):
    """消息创建结果"""
    status: str = Field(..., description="'created' 或 'accepted'", json_schema_extra={"example": "accepted"})
    message_id: int = Field(..., description="消息 ID")
    upload_tasks: List[UploadTaskInfo] = Field(default_factory=list, description="生成的上传任务清单")
